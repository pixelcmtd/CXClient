...
public class SaveFormatOld implements ISaveFormat
{
    ...
    {
        File saveDir = new File(savesDirectory, saveName);
        ...
        else
        {
            ...
            if (file.exists())
                try
                {
                    NBTTagCompound nbttagcompound2 = CompressedStreamTools.readCompressed(new FileInputStream(file));
                    NBTTagCompound nbttagcompound3 = nbttagcompound2.getCompoundTag("Data");
                    return new WorldInfo(nbttagcompound3);
                }
                catch (Exception exception1)
                {
                    logger.error((String)("Exception reading " + file), (Throwable)exception1);
                }

            file = new File(saveDir, "level.dat_old");
    ...
    {
        File dir = new File(this.savesDirectory, dirName);
        ...
        if (dir.exists())
        {
            ...
            if (file.exists())
                try
                {
                    NBTTagCompound nbt1 = CompressedStreamTools.readCompressed(new FileInputStream(file));
                    NBTTagCompound nbt2 = nbt1.getCompoundTag("Data");
                    nbt2.setString("LevelName", newName);
                    CompressedStreamTools.writeCompressed(nbt1, new FileOutputStream(file));
                }
                catch (Exception e)
                {
                    e.printStackTrace();
